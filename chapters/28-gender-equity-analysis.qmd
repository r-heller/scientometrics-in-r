...
subtitle: "Measuring Representation and Disparities in Scholarly Output"
author: "Raban Heller"
date: last-modified
abstract: |
  Gender disparities persist in scholarly publishing — in authorship
  rates, citation patterns, and first/last author positions. This
  chapter demonstrates how to analyse gender representation using
  name-based inference, examines authorship position patterns, and
  discusses the substantial ethical and methodological limitations
  of gender analysis in bibliometrics.
keywords: [scientometrics, gender-analysis, equity, authorship, representation, name-inference]
---

## Learning objectives {#sec-gender-objectives}
...
## Conceptual background {#sec-gender-background}
...
## Worked example {#sec-gender-example}
...
authors <- works |>
  ...
  select(work_id, author_name = author_au_display_name,
         position = author_au_id) |>
  mutate(
...
position_data <- works |>
  ...
  ungroup() |>
  mutate(first_name = str_to_lower(str_extract(author_au_display_name, "^\\S+"))) |>
  mutate(
    gender_proxy = case_when(
      first_name %in% common_female ~ "female",
      first_name %in% common_male ~ "male",
...
## Diagnostics and interpretation {#sec-gender-diagnostics}
...
## Limitations and responsible use {#sec-gender-limitations}
...
## Common pitfalls {#sec-gender-pitfalls}
...
## Exercises {#sec-gender-exercises}
...
## Further reading {#sec-gender-reading}
...
## Session info {#sec-gender-sessioninfo}
