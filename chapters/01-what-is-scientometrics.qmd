...
subtitle: "Defining the Field and Its Scope"
author: "Raban Heller"
date: last-modified
abstract: |
  This opening chapter defines scientometrics, bibliometrics, and informetrics,
  traces the field's history from Garfield's citation indexes to the modern
  open-data era, and outlines the scope of this book. A hands-on worked
  example fetches a small OpenAlex sample to produce basic descriptive
  statistics — publication counts, top journals, and a publication trend.
keywords: [scientometrics, bibliometrics, informetrics, history, openalex]
---

## Learning objectives {#sec-what-objectives}
...
## Conceptual background {#sec-what-background}
...
## Worked example {#sec-what-example}
...
Each row is a scholarly work. Key columns include `display_name` (the title), `publication_date`, `cited_by_count`, `doi`, and `so` (the source or journal name).
...
## Diagnostics and interpretation {#sec-what-diagnostics}
...
cat(glue("Unique journals: {n_distinct(works$so, na.rm = TRUE)}\n"))
```

## Limitations and responsible use {#sec-what-limitations}
...
## Common pitfalls {#sec-what-pitfalls}
...
## Further reading {#sec-what-reading}
...
## Session info {#sec-what-sessioninfo}
