...
subtitle: "Positioning Local Research on the Global Map of Science"
author: "Raban Heller"
date: last-modified
abstract: |
  Science maps provide a global view of the research landscape, and
  overlay maps position a specific institution, journal, or research
  group onto that landscape. This chapter covers the construction of
  journal-level science maps using citation data, the creation of overlay
  maps to visualise research profiles, and interpretation of mapping
  outputs. The worked example builds a journal co-citation map and
  overlays a university's publication profile.
keywords: [scientometrics, science-mapping, overlay-maps, journal-networks, research-profile]
---

## Learning objectives {#sec-scimap-objectives}
...
## Conceptual background {#sec-scimap-background}
...
## Worked example {#sec-scimap-example}
...
cat(glue("Unique source journals: {n_distinct(journal_data$so)}\n"))
```

```{r}
#| label: journal-cocitation
journal_cited <- journal_data |>
  select(citing_journal = so, cited_id)
...
overlay_sizes <- tibble(journal = V(g_journals)$name) |>
  left_join(inst_journals_matched, by = c("journal" = "so")) |>
...
## Diagnostics and interpretation {#sec-scimap-diagnostics}
...
## Limitations and responsible use {#sec-scimap-limitations}
...
## Common pitfalls {#sec-scimap-pitfalls}
...
## Exercises {#sec-scimap-exercises}
...
## Further reading {#sec-scimap-reading}
...
## Session info {#sec-scimap-sessioninfo}
