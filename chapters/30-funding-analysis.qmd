...
subtitle: "Tracing Research Investments to Scholarly Outputs"
author: "Raban Heller"
date: last-modified
abstract: |
  Funding acknowledgements in publications provide a link between
  research investment and scholarly output. This chapter extracts
  funder information from OpenAlex, computes funder-level publication
  and citation statistics, and analyses the relationship between
  funding and research output. We discuss coverage limitations and
  the challenges of attributing impact to specific funding sources.
keywords: [scientometrics, funding-analysis, funder, grant, openalex, research-investment]
---

## Learning objectives {#sec-funding-objectives}
...
## Conceptual background {#sec-funding-background}
...
## Worked example {#sec-funding-example}

### Extracting funder data

```{r}
#| label: fetch-funding-data
#| cache: true
works <- oa_fetch(
  entity = "works",
...
funded <- works |>
  select(id, display_name, cited_by_count, grants) |>
  filter(map_lgl(grants, \(g) !is.null(g) && length(g) > 0))

cat(glue("Works with funder data: {nrow(funded)} / {nrow(works)} ({scales::percent(nrow(funded)/nrow(works))})\n"))
```

```{r}
#| label: unnest-funders
funder_data <- funded |>
  unnest(grants, names_sep = "_") |>
  select(work_id = id, funder = grants_funder_display_name,
         cited_by_count)
...
#| fig-height: 3.5
works |>
  mutate(has_funding = map_lgl(grants, \(g) !is.null(g) && length(g) > 0)) |>
...
## Diagnostics and interpretation {#sec-funding-diagnostics}
...
## Limitations and responsible use {#sec-funding-limitations}
...
## Common pitfalls {#sec-funding-pitfalls}
...
## Exercises {#sec-funding-exercises}
...
## Further reading {#sec-funding-reading}
...
## Session info {#sec-funding-sessioninfo}
