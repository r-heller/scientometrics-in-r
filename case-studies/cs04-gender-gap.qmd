...
subtitle: "Analysing Authorship Patterns Across Disciplines"
author: "Raban Heller"
date: last-modified
abstract: |
  This case study applies the gender analysis methods from Chapter 28
  to compare authorship patterns across two journals in different
  disciplines. We examine gender representation in first and last
  author positions, track temporal trends, and critically evaluate
  the limitations of name-based gender inference.
keywords: [case-study, gender-gap, authorship, equity, name-inference]
---
...
authors <- works_all |>
  select(work_id = id, journal, author) |>
  ...
  ungroup() |>
  mutate(
    first_name = str_to_lower(str_extract(author_au_display_name, "^\\S+")),
    gender = case_when(
